...
void FiniteStateMachine(GameInfoExt_t *game_info_ext) {
  switch (game_info_ext->state) {  // свитч по состояниям

    case kStart:  // если состояние в ожидании запуска
      if (game_info_ext->new_input)
        switch (
            game_info_ext
                ->input) {  // если пришел новый ввод от игрока свитч по вводу
    ...
    case kMoving:
      if (game_inf_ex->new_input) switch (game_info_ext->input) {
          case Left:
